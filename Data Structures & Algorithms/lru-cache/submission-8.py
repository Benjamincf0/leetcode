class LRUCache:
    ...
    def __str__(self):
        if self.root is not None:
            return f"current cache ({self.size}/{self.capacity}) = "+ " ".join([f"{node._key}:{node.val}" for _, node in zip(range(self.size), self.root)]) \
            + f"\n {' '.join([f'{key}:{node.val}' for key, node in self.hm.items()])}"
        else: return "current cache EMPTY"
